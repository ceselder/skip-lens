...
def extract_explanation(response: str) -> str | None:
    ...
    # Fallback for TAGLESS AV policies (e.g. a raw future-lens init that emits a
    # plain continuation, not <explanation>...</explanation>): the whole response
    # IS the explanation. Keeps the autoencoder faithful (h -> raw text -> h) and
    # matches AR-SFT data built with explanation=raw. Only truly empty -> None.
