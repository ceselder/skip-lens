...
def main():
    ...
    p.add_argument("--vllm-quantization", default=None,
                   help="vLLM weight quantization (e.g. 'fp8'). Halves the ~54GB "
                        "rollout copy so the 3 resident model copies (vLLM + HF "
                        "actor + critic) fit a 140GB H200. bf16 weights pushed via "
                        "the IPC sync are re-quantized by vLLM's load_weights.")
    ...
    if getattr(args, "vllm_quantization", None):
        _vllm_extra["quantization"] = args.vllm_quantization
        print(f"[vllm] quantization={args.vllm_quantization} (rollout weights quantized; "
              f"training actor/critic stay bf16)", flush=True)
    llm = VLLM(
        **_vllm_extra,
